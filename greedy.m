function [Accuracy,Match_Level_ACC]=greedy(patterns,nData,test_sample)
...
% [A,B]=size(assignment);
% site=sprintf('./Features/assignment%d.txt',num);
% fid = fopen(site, 'wt');
% for j=1:B
% fprintf(fid, '%d ', assignment(:,j) );
% end
% fclose(fid);
...
% end
% h = figure( 1 ) ;
%  saveas( h , [ './result/Greedy/Greedy' num2str( test_sample(num)) , '.jpg' ] )
