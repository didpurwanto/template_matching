...
% %  testLabel{1,i}   = Label ;
% end
% [assignment]=greedy(patterns,Ntest,test_sample);
...
fid = fopen(site, 'wt');
for i=1:nData
sf3=sprintf('./Features/assignment%d.txt',M(i));
F3=importdata(sf3);
[A,B]=size(F3);
for j=1:B
fprintf(fid, '%d ', F3(1,j) );
end
fprintf(fid, '\n' );
F3=[];
end
...
sf=sprintf('./Features/trainData.txt');
Fttt=importdata(sf)
